...
\begin{center}
\small (Directions are for Cambridge, 22 March, 11:00\,AM)
\end{center}
...
\begin{itemize}
    ...
    \item The galaxy contains \textbf{200--400 billion} such coins
\end{itemize}
...
\smallskip
\small Artist's impression based on Gaia data\\
\tiny Credit: ESA/Gaia/DPAC, S.\,Payne-Wardenaar
\end{center}
\end{column}
\end{columns}
...
\begin{column}{0.55\textwidth}
\begin{itemize}
    \item Space telescope launched by \textbf{ESA} in Dec 2013
    \item Was at \textbf{L2}, 1.5 million\,km from Earth
    \item Operated \textbf{11 years} (double the planned 5!)
    \item Last observation: Jan 2025. Retired: Mar 2025
    \item Measured for each star:
    \begin{itemize}
        \item \textbf{Position} (very precisely)
        \item \textbf{Distance} (using parallax)
        \item \textbf{Motion} (speed and direction)
        \item \textbf{Colour and brightness}
    \end{itemize}
\end{itemize}
\end{column}
...
\begin{tikzpicture}[scale=0.85]
    % Sun
    \shade[ball color=yellow] (-5,0) circle (0.8);
    \node[below] at (-5,-1) {\textbf{Sun}};

    % Earth
    \shade[ball color=blue!60] (1.5,0) circle (0.35);
    \node[below] at (1.5,-0.6) {\textbf{Earth}};

    % L2 point
    \fill[red] (3.8,0) circle (0.08);
    \node[above] at (3.8,0.3) {\textbf{L2}};
    \node[below, font=\small] at (3.8,-0.3) {1.5\,M\,km};

    % Gaia
    \node[font=\small, color=red!70!black] at (4.6,0.8) {Gaia};
    \draw[->, red!70!black, thick] (4.4,0.6) -- (3.95,0.15);

    % orbit hint
    \draw[dashed, red!50, thick] (3.8,0) ellipse (0.5 and 0.7);

    % Arrow Sun-Earth
    \draw[<->, thick, gray] (-4.1,0.9) -- (1.1,0.9);
    \node[above, font=\small, gray] at (-1.5,0.9) {150\,M\,km};

    % L1 point (for context)
    \fill[gray] (-0.3,0) circle (0.06);
    \node[below, font=\scriptsize, gray] at (-0.3,-0.3) {L1};
\end{tikzpicture}
\end{center}
...
\begin{itemize}
    ...
    \item Earth, Sun, and Moon were all behind Gaia's sunshield
\end{itemize}
...
\begin{column}{0.45\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{images/gaia_scanning.png}
...
\smallskip
\small Gaia's all-sky colour map\\
\tiny Credit: ESA/Gaia/DPAC
\end{center}
\end{column}
\end{columns}
